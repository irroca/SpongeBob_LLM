...
def train_epoch(
    epoch, start_step, global_step, policy, ref, optimizer, scaler, loader, args, ctx, wandb,
    ...
    for step, batch in enumerate(loader):
        ...
        if recorder is not None:
            recorder.add_tokens(int(cM.sum() + rM.sum()))
        if (step + 1) % args.accumulation_steps == 0:
            grad_norm = optimizer_step(policy, optimizer, scaler, args.grad_clip)
            pending = False
            global_step += 1
        ...
        if step % args.log_step == 0:
            ...
            if recorder is not None:
                recorder.log(global_step, epoch=epoch + 1, dpo_loss=current_loss, lr=lr, grad_norm=grad_norm)
        ...
        if val_loader is not None and should_evaluate(global_step, args):
            stats = evaluate_preference(policy, ref, val_loader, args.device, args.beta, ctx,
                                        args.val_batches or None)
            print(f"  val: accuracy={stats['accuracy']:.3f} margin={stats['margin']:+.4f} "
                  f"(chosen {stats['chosen_reward']:+.4f} / rejected {stats['rejected_reward']:+.4f})")
            if recorder is not None:
    ...
    if flush_pending_grads(policy, optimizer, scaler, args.grad_clip, pending):
        global_step += 1
